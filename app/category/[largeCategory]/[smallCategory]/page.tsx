...
type Props = {
    params:Promise<{largeCategory:string,smallCategory:string}>
}


export default async function CategoryPage({params}:Props){
    const {largeCategory,smallCategory} = await params
    const decordedLargeCategory = decodeURIComponent(largeCategory)
    const decordedSmallCategory = decodeURIComponent(smallCategory)
   ...
   return (
            ...
            largeCategory={decordedLargeCategory}
            smallCategory={decordedSmallCategory}
            quizzes={quizzes}
            />
    )
}