import {Card,FormControl,FormControlLabel,FormHelperText,
    FormLabel,Radio,RadioGroup,TextField,Checkbox,
    FormGroup,Button,
...
type Props = {
    ...
    index:number,
    formState:FormState<Quiz>,
    isCorrect : boolean,
    deleteChoice : (index:number) => void,
    toggleChoiceIsCorrect:(index:number) => void
}

function ensureArrayChoicesChoiceText(formState:FormState<Quiz>,index:number){
...
function ensureArrayChoicesIsCorrect(formState:FormState<Quiz>,index:number){
    const choices = formState.errors.choices
    if (Array.isArray(choices)) {
        return choices.at(index)?.choiceText?.message?.toString?.()
    }
...
export function ChoiceInputForUpdate({register,index,formState,isCorrect,deleteChoice,toggleChoiceIsCorrect}:Props){
