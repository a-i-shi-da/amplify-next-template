...
import {createQuizAction} from '@/app/_action/createQuiz'
import {QuizForCreate,client_create_schema,Mode} from '@/app/_types/quiz'
...
export default function CreateQuizForm(){
    const [choicesNumber,setChoicesNumber] = useState<number>(4)
    
    const {register,handleSubmit,control,formState,reset,getValues} = useForm<QuizForCreate>({
        resolver : zodResolver(client_create_schema),
        defaultValues: {
            //userIdはなし
            largeCategory : "",
            smallCategory : "",
            ...
            correctCount : 1,
            selectCount : 1,
            displaySelectCount:false,//まだ機能を作成していない
            isPublic : true,
            explanationText : "", //まだ機能を作成していない
            hintText : "",//まだ機能を作成していない
        }
    })
    ...
    const submitHandler = async(quiz:QuizForCreate) => {
        try {
            client_create_schema.parse(quiz)
    ...
    //プレビュー対応
    const compseQuiz = () =>{
        const q:Quiz = {...getValues(),quizId:"",updatedAt:""} 
        return q
    }
    ...
    const [createResult,actionHandler,_isPending] = useActionState<CreateResult<Quiz>,QuizForCreate>(createQuizAction,{})
    ...
    return (
        <div>
            ...
            <BasicModal open={open} handleClose={handleClose}>
                <AnswerQuizForm quiz={compseQuiz()}></AnswerQuizForm>
            </BasicModal>
